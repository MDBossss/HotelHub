    ...
    <div className="payment-info">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            <div className="field">
                ...
                <input type='text' placeholder='Your name' {...register("cardholderName", {required: "Cardholder name is required"})} maxLength={50}/>
                {errors.cardholderName && <p className='error'>{errors.cardholderName.message}</p>}
            </div>
            <div className="row">
                <div className="field">
                    <label>Card Number</label>
                    <input type='text' placeholder='1111 2222 3333 4444' {...register("cardNumber", {required: "Card number is required"})} maxLength={19} minLength={16}/>
                    {errors.cardNumber && <p className='error'>{errors.cardNumber.message}</p>}
                </div>
                <div className="field">
                    <label>Expiry Date</label>
                    <input type="text" placeholder='01/24' {...register("expiryDate", {required: "Expiry date is required"})} maxLength={5}/>
                    {errors.expiryDate && <p className='error'>{errors.expiryDate.message}</p>}
                </div>
                <div className="field">
                    <label>CVV</label>
                    <input type='text' placeholder='123' {...register("cvv", {required: "CVV is required"})} minLength={3} maxLength={3}/>
